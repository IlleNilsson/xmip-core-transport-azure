...
//! Both sides are here — a Location signs, azure-blob's session verifies —
//! because verifying is the same computation with the HMAC's own
//! constant-time check at the end. azure-blob carried this file until the
//! owner's ruling of 2026-09-22 put what Azure speaks in the Azure crate
//! (ADR-0044).
//!
...
impl Signer {
    ...
    pub fn new(account: &str, key_base64: &str) -> Result<Self> {
        let key = base64::decode(key_base64.trim())
    ...
    pub fn verify(&self, request: &Request) -> Result<()> {
        ...
        let differs = || protocol_error("a signature that does not match");
        let signature = authorization
            .strip_prefix("SharedKey ")
            .and_then(|rest| rest.split_once(':'))
            .filter(|(account, _)| *account == self.account)
            .map(|(_, signature)| signature)
            .ok_or_else(differs)?;
        let given = base64::decode(signature).map_err(|_| differs())?;
        self.mac(&string_to_sign(&self.account, request))
            .verify_slice(&given)
            .map_err(|_| differs())
    }

    fn signature(&self, to_sign: &str) -> String {
        base64::encode(&self.mac(to_sign).finalize().into_bytes())
    }

    /// The HMAC under the account key: finalized it is the signature, and
    /// its check compares one in constant time.
    fn mac(&self, to_sign: &str) -> Hmac<Sha256> {
        let mut mac =
